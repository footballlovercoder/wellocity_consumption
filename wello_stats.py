...
uf = st.sidebar.file_uploader("Upload sale data in csv")
st.sidebar.text(' ')
st.sidebar.text(' ')
uf1=st.sidebar.file_uploader("Upload stock data in excel")
...
def load_data():
        ...
        df = pd.read_csv(uf)
        stock=pd.read_excel(uf1)
        return df,stock

if uf is not None and uf1 is not None:    
    df,stock = load_data()
